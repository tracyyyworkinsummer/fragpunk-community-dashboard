...
function columns_(headers){return{id:pick_(headers,['KOC DC ID']),channel:pick_(headers,['Channel Link']),platform:pick_(headers,['Main Platform']),region:pick_(headers,['受众地区','主要受众','受众市场','Region']),kind:pick_(headers,['Video/Stream']),follower:pick_(headers,['Follower']),week:pick_(headers,['Week']),video:pick_(headers,['Video link','Posting Link']),topic:pick_(headers,['Topic']),views:pick_(headers,['Views','Views/UV']),ccv:pick_(headers,['CCV']),engagement:pick_(headers,['Engagement Rate','互动率']),keyword:pick_(headers,['是否包含专项关键词']),valid:pick_(headers,['是否为有效专项']),salary:pick_(headers,['Salary','Total Salary']),fragcoin:pick_(headers,['FragCoin'])};}
